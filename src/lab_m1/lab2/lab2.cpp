...
void Lab2::Init()
{
    ...
    // by definition!
    {
        ...
        vector<VertexFormat> vertices{
            // Bonus 1
            VertexFormat(p1, blue),
            VertexFormat(p2, blue),
            VertexFormat(p3, blue),
            VertexFormat(p1, red),
            VertexFormat(p2, red),
            VertexFormat(p4, red),
            VertexFormat(p2, yellow),
            VertexFormat(p3, yellow),
            VertexFormat(p4, yellow),
            VertexFormat(p3, green),
            VertexFormat(p1, green),
            VertexFormat(p4, green)
        };
    ...
    // Bonus 2
    {
        ...
        float angle = 0, division = 2 * glm::pi<float>() / n;
        glm::vec3 lastPoint(0, radius, 0);
        ...
        for (int i = 0; i < n; i++) {
            ...
            vertices.push_back(VertexFormat(center, glm::vec3(1, 0, 0)));
            vertices.push_back(VertexFormat(glm::vec3(x, y, center.z), glm::vec3(0, 1, 0)));
            vertices.push_back(VertexFormat(lastPoint, glm::vec3(0, 0, 0)));

            indices.push_back(3 * i);
            indices.push_back(3 * i + 1);
            indices.push_back(3 * i + 2);

            lastPoint = glm::vec3(x, y, center.z);
        }
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexNormal"], glm::vec3(2.0f, 0.5f, 0), glm::vec3(0.25f));

    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(-0.5f, 0.5f, 0), glm::vec3(0.25f));

    // TODO(student): Draw the square
    RenderMesh(meshes["square"], shaders["VertexNormal"], glm::vec3(2.0f, 1.5f, 0), glm::vec3(0.25f));

    // TODO(student): Disable face culling
    glDisable(GL_CULL_FACE);

    // Bonus 2
    RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(-2.0f, 2.5f, 0), glm::vec3(0.25f));
}
